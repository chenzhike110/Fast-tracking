...
class YOLO(object):
    ...
    def detect_image(self, image):
        ...
        font = ImageFont.truetype(font='yolo/simhei.ttf',size=np.floor(3e-2 * np.shape(image)[1] + 0.5).astype('int32'))
    ...
    def detect_image_without_draw(self, image):
        ...
        for i, c in enumerate(top_label):
            ...
            result.append(np.array([int((left+right)/2), int((top+bottom)/2), int(right-left), int(bottom-top),int(c)]))
