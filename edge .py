...
def offside_dectet(image, direction, ofplayer_x, ofplayer_y, dfplayer_x, dfplayer_y):
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, 50, minLineLength=min(edges.shape[0], edges.shape[1])/3,
    ...
    if lines is None:
        ...
        for line in lines:
            ...
            if debug == 1:
                cv2.line(img_origin, (x1*shrink1*shrink2, y1*shrink1*shrink2), (x2*shrink1*shrink2, y2*shrink1*shrink2), (0, 0, 255), 2)  # 画线
