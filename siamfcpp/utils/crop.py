...
def get_crop(im,
    ...
    """
    wc = target_sz[0] + context_amount * np.sum(target_sz)
    hc = target_sz[1] + context_amount * np.sum(target_sz)
    ...
    im_crop = func_get_subwindow(im, target_pos, x_size, np.round(s_crop),
                                 avg_chans)
